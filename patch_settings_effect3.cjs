...
const regex = /\/\/ Removed auto-updating form on user change to prevent wiping input\.\n  \/\/ The useState initialization is sufficient because SettingsView remounts when opened\./;

const newEffect = `useEffect(() => {
    // Only update if we haven't properly initialized yet.
    if (user && user.id && !formData.email && user.email) {
      setFormData(prev => ({
        ...prev,
...
console.log('patched SettingsView effect 3');
